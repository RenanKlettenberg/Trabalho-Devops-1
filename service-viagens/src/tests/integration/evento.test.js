...
describe("Editar evento", () => {
    it("deve retornar 200 - SUCESSO", async () => {
        ...
        const res = await api.put('3001', 'evento/', data);

        expect(res.status).toBe(200)
    })

    it("deve retornar 400 - DADO INVÁLIDO", async () => {
        const res = await api.put('3001', 'evento/', {});
...
describe("Deletar evento", () => {
    it("deve retornar 200 - SUCESSO", async () => {
        const res = await api.delete('3001', 'evento/', { eve_id: 1 });

        expect(res.status).toBe(200)
    })

    it("deve retornar 400 - DADO INVÁLIDO", async () => {
        const res = await api.delete('3001', 'evento/', {});
